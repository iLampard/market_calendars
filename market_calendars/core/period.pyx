...
from .assert_utils cimport py_assert, py_ensure_raise

unit_pattern = re.compile('[BbDdMmWwYy]{1}')
number_pattern = re.compile('[-+]*[0-9]+')

units_dict = {'d': TimeUnits.Days,
              'b': TimeUnits.BDays,
              'w': TimeUnits.Weeks,
              'm': TimeUnits.Months,
              'y': TimeUnits.Years}
...
cdef class Period(object):
    def __init__(self, str reprStr=None, int length=0, int units=0):
        ...
        if reprStr:
            units_pos = unit_pattern.search(reprStr)
            num_pos = number_pattern.search(reprStr)
            units_str = reprStr[units_pos.start():units_pos.end()].lower()
            n = int(reprStr[num_pos.start():num_pos.end()])
            self._length = n
            self._units = int(units_dict[units_str])
        else:
            self._length = length
            self._units = units
    ...
    cpdef Period normalize(self):
        ...
        if length != 0:
            if units == TimeUnits.BDays or units == TimeUnits.Weeks or units == TimeUnits.Years:
                ...
                py_ensure_raise(TypeError, "wrong period units type {0}".format(units))
...
cdef bint _lt_cmp(Period p1, Period p2) except -1:
    ...
    if p1lim[1] < p2lim[0]:
        ...
        py_ensure_raise(ValueError, "undecidable comparison between {0} and {1}".format(p1, p2))

cdef tuple _days_min_max(Period p):
    ...
    if units == TimeUnits.Days:
        ...
        py_ensure_raise(ValueError, "Business days unit has not min max days")
